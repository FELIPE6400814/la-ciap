document.addEventListener("DOMContentLoaded", function () {
  ...
  // Funcionalidad para el aviso de cookies eliminada
  ...
  if (mobileMenuButton && mobileMenu) {
    ...
    if (navList && navLinks.length > 0) {
        ...
        const observerCallback = (entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (id) {
                        const newActiveLink = document.querySelector(`.desktop-nav li a[href="#${id}"]`);
                        if (newActiveLink) {
                            activeLink = newActiveLink;
                            // Only update if not currently hovering (optional, but good for UX)
        ...
        // Initial check for active section on load (optional, or rely on scroll)
        // setTimeout(() => {
        //     const startSection = document.querySelector('section'); // Simple check
        //     if(startSection) observerCallback([{target: startSection, isIntersecting: true}]); 
        // }, 100);
    }

  const fadeElements = document.querySelectorAll(
    "section, .feature-item, .investigator-item, footer"
  ); // Selecciona todas las secciones y otros elementos que quieres animar
